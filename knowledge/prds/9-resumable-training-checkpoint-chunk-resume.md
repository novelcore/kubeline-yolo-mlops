...
| F-03 | Run-state manifest writer | Alexa-Aposto | M | P0 | No | D-02 | 3.2 | Training service writes/updates `run_state.json` atomically at run start and every checkpoint interval: MLflow run ID, last epoch, checkpoint index with hashes, dataset hash, config hash, workflow UID (from the env F-01 injects) + heartbeat timestamp (T-05) |
| F-04 | MLflow run lineage (v1) / continuation (follow-up) | Alexa-Aposto | M | P0 | No | D-03 | 3.3 | Own the MLflow run lifecycle: create the run before training and write its ID to run_state at start; v1 creates a new run per resume tagged `resumed_from` + attempt + source workflow UID (D-03 B); single-run continuity (A) follows once the callback param-collision fix (skip/normalize `trainer.args` re-log on resume) is proven |
| F-05 | Chunk-aware idempotent dataset loading | Alexa-Aposto | M | P0 | No | D-04 | 3.4 | Deterministic chunked downloads with completion markers, `dataset.chunk_size` config (default 500), extended manifest with per-chunk keys and `manifest_sha256`; re-runs skip complete chunks. Full-download/local modes only — streaming and labels-only enforce `manifest_sha256` without markers |
...
| 6 | Ownership split: F-01/F-06 are operator-repo work (kubecore-operator k8smlapp composition); who implements F-02–F-05/F-07/F-08 in kubeline-yolo-mlops, and in what sequence relative to the operator-side changes? | Two repos, two release trains (operator OCI Configuration package vs pipeline images); the dependency chain (§12) crosses the boundary at F-01/F-06 | meter-peter | before implementation start | resolved (meter-peter, 2026-06-12) | Platform/wiring vs training-internals split. meter-peter (platform): F-01, F-06 (operator composition), F-02 (validation wiring), F-07, F-08. Alexa-Aposto (training internals): F-03 (run-state writer), F-04 (MLflow run lifecycle + callback fix), F-05 (deterministic chunking). The §12 chain sequences cleanly across the boundary: Alexa-Aposto's F-03+F-04 → F-05 land first, then meter-peter's F-02 → F-01 → F-06 wiring consumes their contracts |
